...
if ( ! class_exists( 'CSF_Shortcoder' ) ) {
  class CSF_Shortcoder extends CSF_Abstract{
    ...
    public function __construct( $key, $params = array() ) {
      ...
      add_action( 'admin_footer', array( &$this, 'add_footer_gesusl_shortcode' ) );
      add_action( 'customize_controls_print_footer_scripts', array( &$this, 'add_footer_gesusl_shortcode' ) );
      add_action( 'wp_ajax_csf-get-shortcode-'. $this->unique, array( &$this, 'get_shortcode' ) );

      if ( ! empty( $this->args['show_in_editor'] ) ) {

        CSF::$shortcode_instances[$this->unique] = wp_parse_args( array( 'hash' => md5( $key ), 'gesusl_id' => $this->unique ), $this->args );
        ...
        if ( CSF::is_active_plugin( 'elementor/elementor.php' ) ) {
          ...
          add_action( 'elementor/editor/footer', array( 'CSF_Field_icon', 'add_footer_gesusl_icon' ) );
          add_action( 'elementor/editor/footer', array( &$this, 'add_footer_gesusl_shortcode' ) );
        }
    ...
    public function add_footer_gesusl_shortcode() {
    ...
    ?>
      <div id="csf-gesusl-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui csf-gesusl csf-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-gesusl-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'csf_shortcode_nonce' ) ); ?>">
          ...
          <div class="csf-gesusl-table-cell">
            ...
            <div class="csf-gesusl-inner">
              <div class="csf-gesusl-title">
                ...
                <div class="csf-gesusl-close"></div>
              </div>
              <?php

                echo '<div class="csf-gesusl-header'. esc_attr( $hide_header ) .'">';
                echo '<select>';
              ...
              <div class="csf-gesusl-insert-wrapper hidden"><a href="#" class="button button-primary csf-gesusl-insert"><?php echo $this->args['insert_title']; ?></a></div>
            </div>
          </div>
        </div>
      </div>
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( CSF::$shortcode_instances as $value ) {
        echo '<a href="#" class="button button-primary csf-shortcode-button" data-editor-id="'. esc_attr( $editor_id ) .'" data-gesusl-id="'. esc_attr( $value['gesusl_id'] ) .'">'. $value['button_title'] .'</a>';
      }
