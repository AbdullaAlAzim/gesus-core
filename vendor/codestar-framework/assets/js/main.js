...
;(function( $, window, document, undefined ) {
  ...
  $.fn.csf_field_icon = function() {
    return this.each( function() {
      ...
      $this.on('click', '.csf-icon-add', function( e ) {
        ...
        var $button = $(this);
        var $gesusl  = $('#csf-gesusl-icon');

        $gesusl.removeClass('hidden');

        CSF.vars.$icon_target = $this;

        if ( !CSF.vars.icon_gesusl_loaded ) {

          $gesusl.find('.csf-gesusl-loading').show();

          window.wp.ajax.post( 'csf-get-icons', {
            nonce: $button.data('nonce')
          }).done( function( response ) {

            $gesusl.find('.csf-gesusl-loading').hide();

            CSF.vars.icon_gesusl_loaded = true;

            var $load = $gesusl.find('.csf-gesusl-load').html( response.content );
            ...
            $load.on('click', 'i', function( e ) {
              ...
              $gesusl.addClass('hidden');

            });

            $gesusl.on('change keyup', '.csf-icon-search', function() {
            ...
            $gesusl.on('click', '.csf-gesusl-close, .csf-gesusl-overlay', function() {
              $gesusl.addClass('hidden');
            });

          }).fail( function( response ) {
            $gesusl.find('.csf-gesusl-loading').hide();
            $gesusl.find('.csf-gesusl-load').html( response.error );
            $gesusl.on('click', function() {
              $gesusl.addClass('hidden');
            });
          });
        }
  ...
  $.fn.csf_shortcode = function() {
    ...
    return this.each( function() {
      ...
      var $gesusl   = $(this),
          $load    = $gesusl.find('.csf-gesusl-load'),
          $content = $gesusl.find('.csf-gesusl-content'),
          $insert  = $gesusl.find('.csf-gesusl-insert'),
          $loading = $gesusl.find('.csf-gesusl-loading'),
          $select  = $gesusl.find('select'),
          gesusl_id = $gesusl.data('gesusl-id'),
          nonce    = $gesusl.data('nonce'),
          editor_id,
          target_id,
          gutenberg_id,
          sc_key,
      ...
      $(document).on('click', '.csf-shortcode-button[data-gesusl-id="'+ gesusl_id +'"]', function( e ) {
        ...
        if ( $gesusl.hasClass('csf-shortcode-single') && sc_name === undefined ) {
      ...
      $select.on( 'change', function() {
        ...
        if ( sc_key ) {
          ...
          window.wp.ajax.post( 'csf-get-shortcode-'+ gesusl_id, {
      ...
      $insert.on('click', function( e ) {
        ...
        var shortcode = '';
        var serialize = $gesusl.find('.csf-field:not(.csf-depend-on)').find(':input:not(.ignore)').serializeObjectCSF();
      ...
      $gesusl.on('click', '.csf--repeat-button', function( e ) {

        e.preventDefault();

        var $repeatable = $gesusl.find('.csf--repeatable');
        ...
        CSF.helper.name_nested_replace( $gesusl.find('.csf--repeat-shortcode'), sc_group );
